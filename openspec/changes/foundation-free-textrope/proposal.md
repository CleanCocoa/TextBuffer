...
- **`Range<Int>` primitives on TextRope.** `content(in:)`, `delete(in:)`, and `replace(range:with:)` become `Range<Int>`-based (half-open, stdlib-only, still UTF-16 code unit offsets — only the range *type* changes, never the unit). `insert(_:at: Int)` already conforms. A new `package`-scoped stdlib-only primitive `utf16CodeUnits(in: Range<Int>)` is added (not public API) so the relocated composed-sequence machinery keeps efficient, surrogate-safe code-unit access (see design.md D4/D5).
...
- `rope-utf16-navigation`: `content(in:)` restated over `Range<Int>`; new `package`-scoped `utf16CodeUnits(in:)` primitive; the composed-sequence read requirement re-homed to the TextBuffer extension layer (semantics unchanged)
...
1. **ADR-013 deviation on composed sequences** *(resolved 2026-08-03)*: deviation confirmed and folded back into the ADR as its "Amendment 2026-08-03" section. NSString composed-sequence segmentation is not stdlib-reproducible (CRLF: NSString yields two sequences, Swift one `Character`; prepend scalars such as U+0600: NSString splits, Swift joins), so a Foundation-free TextRope implementation cannot honor the NSString-parity contract that `fix-composed-sequence-reads` entrenches at both rope and buffer level. The composed APIs stay NSRange-based and relocate wholly to the TextBuffer extension layer; no Swift-grapheme-semantics twin is added (zero consumers would exist for one). The explicit cost — the standalone TextRope product cannot answer NSString-parity cluster queries — is recorded in the amendment.

2. **Shape of the code-unit primitive** *(resolved 2026-08-03)*: block form confirmed — `utf16CodeUnits(in: Range<Int>) -> [UTF16.CodeUnit]` (block reads amortize tree descents; safe on mid-surrogate ranges where `content(in:)` is not; the scalar-accessor alternative is O(log n) per unit during regional-indicator run walks). Additionally decided: the primitive is **`package`-scoped, not public** — its only consumer is the same-package TextBuffer target, so it adds nothing to the library's semver surface and stays freely changeable. This also resolves the concern that supporting the relocated machinery would widen TextRope's public API (see design.md D5 and Risks); UTF-8 storage per ADR-004 stands, with UTF-16 access remaining a per-chunk boundary conversion.
