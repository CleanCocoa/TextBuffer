...
- [x] 4.2 Implement `Node.ensureUniqueChild(at:)` — check `isKnownUniquelyReferenced(&children[index])` in place and replace the child with a `shallowCopy()` when shared (text corrected at review-fold: shipped as in-place check on the array element, not extract→check→write-back; behavior pinned by `testEnsureUniqueChildCopiesSharedChild` and `testEnsureUniqueChildKeepsUniqueChild`)
...
- [x] 5.5 Write `Tests/TextRopeTests/TextRopeConstructionTests.swift` — test: empty init round-trip; small string single-leaf; large string multi-leaf with correct chunk sizes; `\r\n` boundary integrity; content round-trip for ASCII, multi-byte, and mixed strings; utf8Count/utf16Count match String properties; Equatable (same content equal, different content not equal) (text corrected at review-fold: the Equatable bullet was never delivered in `TextRopeConstructionTests.swift` — the file ships without rope-to-rope `==` assertions; the coverage now lives in `Tests/TextRopeTests/TextRopeEqualityTests.swift`, added by `fix-rope-cow-and-equality-coverage`)
