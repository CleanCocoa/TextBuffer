...
final class TextRopeStressTests: XCTestCase {
    ...
    private func runStressTest(seed: UInt64, operations: Int, validateEveryOperation: Bool = false) {
        ...
        for i in 0..<operations {
            ...
            if validateEveryOperation {
                // Combining marks make Swift `String ==` (canonical equivalence) weaker
                // than byte equality; the dedicated per-operation seed pins byte-exact
                // fidelity so a transposition cannot hide behind an equivalent reordering.
                XCTAssertTrue(
                    Array(rope.content.utf8) == Array(string.utf8),
                    "UTF-8 byte sequence mismatch at \(context), op=\(op)"
                )
            }
