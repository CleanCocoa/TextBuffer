...
- `RopeBuffer` and `SendableRopeBuffer`: `lineRange(for:)` and `wordRange(for:)` no longer materialize the full document on every call. Both queries now read only windows around the search range via rope descent — `lineRange` scans for the delimiters bounding the line in fixed-size blocks (Foundation's exact delimiter set: LF, CR, CRLF as one delimiter with longest match preferred, NEL `U+0085`, LS `U+2028`, PS `U+2029`), and `wordRange` runs the unchanged word classification over a window that doubles whenever a result touches a window edge or a whitespace scan is inconclusive. Per-call cost drops from O(n) in document length to O(log n + result length) — the degenerate case being a delimiter-free document, whose line *is* the document. Results and thrown bounds errors are unchanged for every input: an oracle-compared drift zoo (delimiter kinds, CRLF straddling chunk seams, document edges, words with apostrophes/hyphens/emoji, whitespace runs wider than the window) pins both buffer types against `MutableStringBuffer`, and 1 MiB vs 4 MiB ratio tests pin size independence (measured `lineRange` 3.99× → 1.06×, `wordRange` 2.19× → 1.02×). Closes DEF-012 fully; `SendableRopeBuffer` previously inherited the materializing protocol defaults silently.

- `RopeBuffer` and `SendableRopeBuffer`: `unsafeCharacter(at:)` no longer pays window materialization and `NSString` bridging for printable-ASCII reads — the dominant case in code and prose, hit per keystroke and per syntax-highlighting scan. When the code unit at the offset and both of its existing neighbors are printable ASCII (`0x20...0x7E`; a missing neighbor at a document edge counts as safe), the read now answers from a single 3-code-unit block read instead of materializing a ±128-unit window and bridging it to `NSString`. Measured per-call on a ~1 MiB ASCII document (release build): 1,492 ns before, 289 ns after — 5.2× faster, narrowing the gap to `MutableStringBuffer` from 10.5× to 2.8×. Results are unchanged at every offset of every document — the every-offset mixed-content drift sweep pins fast-path/windowed-path agreement against `MutableStringBuffer` — and every non-ASCII, CR-, or LF-adjacent read keeps the windowed expansion path unchanged (emoji-heavy and flag-run documents measure within run-to-run variance of their baselines).
