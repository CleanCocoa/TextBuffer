...
DEF-015 and DEF-016 were filed against the not-yet-pushed 0.10.0 (`438c041`) by the 2026-08-04 post-release review; both repros verified against that tag; both **resolved in 0.10.1** (2026-08-04). Their line references describe 0.10.0 code. DEF-017, discovered during the DEF-016 fix, was **fixed on the 0.11.0 train** (2026-08-04, change `fix-delete-merge-starvation`); DEF-011's read fast path followed on the same train (2026-08-04, change `perf-read-fast-path`), and DEF-012's O(log n) queries closed last on the same train (2026-08-04, change `rope-log-queries`). DEF-018 (equality-contract hybrid) was filed 2026-08-06 against 0.11.0 and **fixed on the same day** (change `fix-equality-contract`); nothing is open.
...
## Decisions (2026-08-06 grilling)

Equality contract, decided with a codex CLI consult and a four-juror Opus tribunal (app-developer, API-design, Unicode-pipeline, maintenance lenses; unanimous, high confidence):

- **`TextRope.==` is code-unit (UTF-8 byte) equality.** Rationale, strongest first: (1) equality must be a congruence for the offset-addressed API — canonical `==` admits pairs with different `utf16Count` that diverge under identical operations, poisoning echo suppression and convergence checks; (2) precedent — swift-collections gives `BigString.UTF16View` code-unit equality (tested: `"Café".utf16 != "Cafe\u{301}".utf16`) and `TextRope`'s public surface *is* the UTF-16 view; every surveyed editor rope (ropey, crop, xi, VS Code, CodeMirror) is code-unit or non-Equatable; (3) canonical `==` has no normalization-invariant summary field, so it deletes the O(1) early-out and forecloses streaming comparison (normalization is not chunk-local — swift-foundation documents this in `AttributedString`); (4) reversibility — widening to canonical later is cheap, narrowing away from it is not; (5) the transfer-convergence spec already demands byte-identical. Rejected: canonical equivalence (silent-late failure modes: dropped edits, blessed divergence), documented hybrid (unstatable contract, breaks buffer interchangeability at the app's 16 KB rope threshold).
- Tier 3 becomes a code-unit comparison (`utf8` lockstep); tiers 1–2 stay, the tier-2 soundness proof now genuinely valid. Streaming leaf-pair zipper remains the later perf slice (DEF-010 ledger), unblocked by this ruling.
- **One dialect package-wide**: op-log types get explicit code-unit `==`; drift assertions and `assertUndoEquivalence` harden to byte-exact content comparison.
- **New named surface**: `isCanonicallyEquivalent(to:)` (the render-equality question, deliberate at the call site) and `isTriviallyIdentical(to:)` (tier 1 exposed under the SE-0494 spelling; O(1) primitive for app-side dirty tracking). The spec's normalization requirement routes callers: render-equality → canonical predicate; ingress policy → Foundation normalization with the CJK-compatibility-ideograph caveat (NFC singleton-decomposes U+F900-range — blanket ingress normalization is a policy choice); range derivation → the existing composed-sequence/word/line expansion APIs.
- **Normalization policy stated as a spec requirement**: TextRope stores and compares bytes exactly and never normalizes on any operation; normalization is a caller-boundary policy. NFC/NFD and CCC-reorder pairs become named *unequal* scenarios.
- **Hashable: not added.** The spec records the constraint that any future conformance must hash exactly the bytes `==` compares (byte hashing is compositional over leaves; canonical hashing is not).
- App-side note (for the announcement, not this repo): no surveyed editor uses content equality for dirty tracking — version counters are the norm; `isTriviallyIdentical(to:)` gives TheArchive2 the O(1) primitive to move toward that.
...
### DEF-018: Equality is an accidental hybrid — canonical-equivalence leakage in `==`, the op log, and the drift oracle — `fixed`

Fixed 2026-08-06 (`876a8a4`, change `fix-equality-contract`): all three manifestations closed in one dialect. (1) `TextRope.==` tier 3 is now `lhs.content.utf8.elementsEqual(rhs.content.utf8)` — tiers 1 and 2 untouched, and the tier-2 soundness proof is now genuinely valid rather than accidentally safe; the CCC-reorder pair is red-first pinned by `testCanonicallyReorderedCombiningMarksAreNotEqual`, the NFC/NFD pair by the labelled early-out pin. The cross-buffer drift is gone: `testEqualityDialectAgreesWithOracleOnCanonicallyReorderedContent` pins `RopeBuffer` against the `MutableStringBuffer` oracle on the exact pair they used to disagree on. (2) `BufferOperation.Kind` has an explicit `==` comparing every text payload with `utf8.elementsEqual`; `BufferOperation`, `UndoGroup`, and `OperationLog` inherit it through their synthesized conformances (`UndoGroup.actionName` stays a Swift `String` comparison by decision — it is a menu label, not recorded text), and `testEveryPayloadOfEveryKindParticipatesInEquality` guards the hand-written comparison's field coverage. (3) `assertDriftMatch` and both `assertUndoEquivalence` overloads (and `assertSendableUndoEquivalence` by delegation) now assert `Array(content.utf8)` alongside the readable `String` assertion. `isCanonicallyEquivalent(to:)` and `isTriviallyIdentical(to:)` land as named predicates for the two questions the canonical dialect was silently answering, and the never-normalize storage guarantee is pinned by `TextRopeNormalizationTests`. The allocation-free leaf-streaming zipper is unblocked by this ruling but stays on the DEF-010 ledger.

Filed 2026-08-06 after the equality-contract analysis (codex consult + 4-Opus tribunal, unanimous verdict; decisions recorded in the "Decisions (2026-08-06 grilling)" section below). Three related manifestations, all verified:

- **`TextRope.==` (`TextRope.swift:50-54`) is a hybrid relation**: summary early-out (code-unit-flavored), then `content ==` (canonical). Verified both ways: `TextRope("é") != TextRope("e\u{301}")` though `String ==` says equal; `TextRope("e\u{301}\u{323}") == TextRope("e\u{323}\u{301}")` though byte-different (equal summaries, CCC reorder). The canonical fallback breaks congruence: two `==` ropes can report different `utf16Count` and diverge under the identical operation at the identical offset — poison for the offset-addressed API and the echo-suppression/convergence consumers (`RopeBuffer.==`, `SendableRopeBuffer.comparator(.content)`). Cross-buffer drift TODAY: `MutableStringBuffer.==` (`NSString.isEqual`) says unequal on the CCC pair where `RopeBuffer` says equal. The spec is internally inconsistent: `rope-core-types/spec.md:106` mandates `content` equality (canonical) while `:114`'s tier-2 soundness proof holds only for code-unit semantics.
- **Operation-log types** (`BufferOperation`, `UndoGroup`, `OperationLog`) use synthesized `Equatable` over `String` members — canonical — so `comparator(.content, .undoHistory)` mixes dialects in one call.
- **Drift-oracle hole**: `RopeBufferDriftTests` (and `assertUndoEquivalence`) compare `content` via `XCTAssertEqual` on `String` — canonical — so a rope-side normalization divergence would pass the oracle suite undetected.

Underlying gap: the repo has no normalization policy anywhere (the word appears in no source, spec, or doc). Ruling (2026-08-06): **code-unit equality** across the package; see Decisions for the full obligation list.
